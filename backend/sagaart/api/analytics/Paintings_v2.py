...
def make_shows_authority_from_shows(shows: str) -> float:
    ...
    содержащей информацию о выставках автора(будь то индивидуальных
    или групповых) в число - эквивалент суммарного посещения выставок
    с весами authorities(пока они все одинаковы). All shows - все
    выставки, имеющиеся у меня на данный момент, в дальнейшем
    будет пополняться.
    """
    all_shows = [
        "centre pompidou",
...
def preprocess(data: list) -> np.ndarray:
    ...
    данных в удобоворимый для модели вид. Все переменные должны
    находиться на таких же местах, что указано
    выше. В данном случае, когда пользователь вводит данные и мы пока
    не представляем, в каком формате они будут представлены,
    потому как модель должна будет заранее знать о
